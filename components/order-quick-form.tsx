...
export function OrderQuickForm() {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm font-outfit">
      <div className="p-5 space-y-4">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? "bg-[#c8143d] text-white border-[#c8143d]"
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="space-y-1">
            ...
            <Input
              ...
              className="h-10 rounded-md border-slate-200 bg-white text-sm font-light"
            />
          </div>
          <div className="space-y-1">
            ...
            <Select value={customerCountryCode} onValueChange={handleCountryChange}>
              <SelectTrigger className="h-10 rounded-md border-slate-200 bg-white text-sm font-light">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-50 rounded-md">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40"
            >
              <span className="text-sm font-medium">−</span>
            </button>
            <span className="w-8 text-center text-base font-semibold text-slate-900">{travelerCount}</span>
            <button
              type="button"
              onClick={() => handleTravelerCountChange(1)}
              disabled={travelerCount >= 5}
              className="flex h-8 w-8 items-center justify-center rounded bg-[#c8143d] text-white hover:bg-[#b01030] disabled:opacity-40"
            >
              <span className="text-sm font-medium">+</span>
            </button>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Passenger Details</span>
            <div className="h-px flex-1 bg-slate-200" />
          </div>
          {travelers.map((traveler, index) => (
            <div key={index} className="p-3 bg-slate-50 rounded-md">
              <div className="flex items-center gap-2 mb-2">
                <span className={cn("text-[10px] font-semibold uppercase tracking-wider", index === 0 ? "text-[#c8143d]" : "text-slate-500")}>
              ...
              <div className="grid gap-2 md:grid-cols-[70px_1fr_1fr]">
                <Select value={traveler.title} onValueChange={(v) => handleTravelerChange(index, "title", v)}>
                  <SelectTrigger className="h-9 rounded-md bg-white text-xs"><SelectValue /></SelectTrigger>
                  <SelectContent>{TITLES.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
                </Select>
                <Input placeholder="First name" value={traveler.firstName} onChange={(e) => handleTravelerChange(index, "firstName", e.target.value)} className="h-9 rounded-md bg-white text-xs" />
                <Input placeholder="Last name" value={traveler.lastName} onChange={(e) => handleTravelerChange(index, "lastName", e.target.value)} className="h-9 rounded-md bg-white text-xs" />
              </div>
            </div>
          ))}
        </div>

        <p className="text-[11px] text-slate-400 text-center">By continuing, you agree to our Terms & Privacy Policy</p>

        <Button onClick={handleSubmit} disabled={!isValid()} className="w-full h-11 rounded-md bg-[#c8143d] hover:bg-[#b01030] font-medium text-sm">
          Continue to Order
